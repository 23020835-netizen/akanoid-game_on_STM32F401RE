...
void Draw_PlayIcon(int16_t x, int16_t y) {
    // Vẽ hình tam giác (Nút Play) bằng các đường dọc ngắn dần
    for (int16_t i = 0; i < 10; i++) {
        DrawFastVLine(x + i, y + i, 20 - 2 * i);
    }
}

void Draw_PauseIcon(int16_t x, int16_t y) {
    // Vẽ 2 vạch dọc (Nút Pause)
    FillRect(x, y, 6, 18);
    FillRect(x + 12, y, 6, 18);
}
